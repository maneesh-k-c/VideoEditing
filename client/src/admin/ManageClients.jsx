...
export default function ManageClients() {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:5000/api/register/all-users`).then((res) => {
            setMedia(res.data.data);
        })
    }, [])

    const deleteMedia = (id) => {
        axios.get(`http://localhost:5000/api/register/delete-user/${id}`).then((res) => {
    ...
    const approveUSer = (id) => {
        axios.get(`http://localhost:5000/api/register/approve-user/${id}`).then((res) => {
            toast.success(res.data.message);
            const filterData = media.filter((item) => {
        ...
        <>
            ...
            <section className="contact_section card-section layout_padding">
                <div className="container text-center">
                    <div className="">
                        <h2 className="" style={{ color: "black" }}>Manage Clients</h2>
                    </div>
                </div>
